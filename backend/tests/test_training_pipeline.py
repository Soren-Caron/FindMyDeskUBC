...
# Ensure backend directory is on sys.path so we can import model.py
...
def test_train_model(tmp_path, monkeypatch):
    """Training pipeline must create lookup.json with correct structure."""
...
def test_predict_busy_score(tmp_path, monkeypatch):
    """Prediction uses lookup + feedback + weather and returns 0–1 score."""
